...
Moreover, divisive hierarchical clustering is *monotone*. At each level (depending on the definition of said 'level'), the subclusters being formed must have a smaller level than the parent cluster. Under these
...
```{r distance_matrix_example, eval=T, echo=T}

set.seed(1234)

normal_data <- rnorm(5, mean = 3, sd = 1.5) |> 
  round() |> 
  as.vector()

# Pairwise Euclidean distances into a matrix
distance_matrix <- data.frame(matrix(0, nrow = length(normal_data), 
                                      ncol = length(normal_data)))


for (i in 1:length(normal_data)) {
  for (j in 1:length(normal_data)) {
...
# Print the distance matrix using kableExtra
...
It seems the split involving observation $10$ occurs at a dissimilarity height much greater than others.
