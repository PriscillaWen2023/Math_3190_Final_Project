...
Divisive or 'top-down' clustering algorithms have, by the past, been studied less extensively than bottom-up or agglomerative clustering (pg. 486, Hastie et al., 2009). Both types of algorithm ***divide a data set into a sequence of nested partitions*** (Gan, 109). They both are also subject to some of the same drawbacks:
...
The diana() clustered object contains a matrix representing the sequence of clustering, where the negative signs represent the point split from the cluster (\`\$merge\` is a misnomer):

```{r diana()_merge_sequence, eval=T, echo=T}
print(hc_euclid$merge)
```

-   **Row 1:** `[-11, -23]`: In the first step, data points with indices -11 and -23 (assuming they were initially in the same cluster) were separated into individual clusters.

-   **Row 4:** `[-7, 2]`: Data point with index -7 was isolated from a cluster, while data point with index 2 remained in the existing cluster.

-   **Row 9:** `[4, 7]`: Clusters with indices 4 and 7 (referring to rows in the `hc_euclid` object) were merged.
...
```{r clustering_tables, eval=T, echo=T}
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    selectInput("distanceType", "Select Distance Type",
                choices = c("euclidean", "manhattan", "minkowski"), selected = "euclidean"),
...
server <- function(input, output, session) {
  ...
  output$dendrogramPlot <- renderPlot({
    ...
    if (!is.null(dist_matrix)) {
      ...
      plot(diana_clust, labels = clusters, main = "Dendrogram of Divisive Hierarchical Clustering")
      rect.hclust(as.hclust(diana_clust), k = input$numClusters, border = 2:6)
    }
  })
}
...
Highest Cophenetic correlation is $0.8676$ , which is obtained with MAXIMUM distance.
...
# Plot dendrogram
plot(best_hc_colored, main = "Dendrogram obtained by dissimilarity from 'maximum' distance")
```
...
best_hc <- diana(dist(data_new, method = "manhattan"))
...
plot(as.dendrogram(best_hc), main = "Dendrogram with MAXIMUM distance (colored)",
...
It appears that observation 19 is an outlier as it is the only observation in Cluster 3. From the dendrogram this point significantly differs from the rest of the data, perhaps due to error in measurement, data entry, or scale. It seems the film *Pom Poko* from director Isao Takahata, has a budget in a lower range but a revenue recorded as $4,470,000,000$, which is on the scale of $\yen$ in 1994, according to IMDB. Noting this, we scale the value, dividing by the average closing price of USD-YEN in 1994, 102.18, to obtain :

```{r data_no_outlier, echo = T, eval = F, warning=F, message = F}

data_new$`Revenue_in_$`[19] <- data_new$`Revenue_in_$`[19]/102.18

# Scale outlier observation (19)
data_scale_outlier <- data_new
# Print the dimensions of the new dataframe
print(dim(data_scale_outlier))
```
...
diana_results <- lapply(distance_metrics, function(distance_metric) {
  ...
  hc2 <- diana(
    x = distance_matrix2,
    diss = TRUE,
    keep.diss = nrow(data_scale_outlier) < 100,
    keep.data = FALSE
  )
...
# Perform hierarchical clustering with the chosen parameters
best_hc2 <- diana(dist(data_scale_outlier, method = "maximum"))

# Cut the dendrogram to get the clusters
clusters2 <- cutree(best_hc2, k = 3)  # You can adjust the number of clusters as needed

# Plot the dendrogram with cluster colors
plot(as.dendrogram(best_hc2), main = "Dendrogram with MAXIMUM distance ('Pom Poko' revenue rescaled)",
     xlab = "Observations", ylab = "Height")
